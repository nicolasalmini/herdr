...
Set `show_tab_numbers = true` under `[ui]` to prefix each desktop tab label with its one-based current-order index, such as `1: shell` or `10: editor`. The numbers follow the same visual order used by indexed `switch_tab` bindings. They update after tabs are created, closed, or moved without changing stored labels, API labels, or session data. The default is `false`.
