...
Set `show_tab_numbers = true` under `[ui]` to show each automatically named desktop tab as its one-based current-order index, such as `1` or `10`, and to prefix custom labels, such as `2: shell`. The numbers follow the same visual order used by indexed `switch_tab` bindings. They update after tabs are created, closed, or moved without changing stored labels, API labels, or session data. The default is `false`.
